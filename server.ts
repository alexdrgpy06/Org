    ...
    try {
      ...
      pythonProcess.on('close', (code) => {
        if (code !== 0) {
          ...
          return res
            .status(500)
            .json({ error: 'An error occurred during the investigation.' })
        }
        ...
        try {
          ...
          res
            .status(500)
            .json({ error: 'Failed to parse investigation results.' })
        }
      })
    } catch (error: any) {
      console.error('OSINT Search Error:', error)
      res.status(500).json({
        error: error.message || 'An error occurred during the investigation.',
      })
    }
  })

    ...
    <div className="bg-neutral-950 text-neutral-200 p-4 font-mono">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-neutral-900 border border-neutral-800 p-6 rounded-lg mb-6">
          ...
          <div>
            ...
            <div className="flex gap-4 mt-2 text-sm text-neutral-400">
              <span className="flex items-center gap-1">
                <FileText size={14} /> CI: {profile.document_id}
              </span>
              {profile.tax_id && (
                <span className="flex items-center gap-1">
                  <Briefcase size={14} /> RUC: {profile.tax_id}
                </span>
              )}
              {profile.address_electoral && (
                <span className="flex items-center gap-1">
                  <MapPin size={14} /> {profile.address_electoral}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="mt-4 md:mt-0 flex flex-col items-end">
          ...
          <div className="flex items-center gap-3">
            ...
            <span
              className={`text-2xl font-bold ${profile.risk_score > 70 ? 'text-red-500' : 'text-yellow-500'}`}
            >
              {profile.risk_score}
            </span>
          </div>
          <button
            onClick={onExportPDF}
            className="mt-4 bg-cyan-600 hover:bg-cyan-500 text-white px-4 py-2 rounded text-sm font-semibold transition-colors flex items-center gap-2"
          >
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-neutral-900 border border-neutral-800 p-4 rounded-lg">
                <div className="text-xs text-neutral-500 flex items-center gap-1 mb-2">
                  <Phone size={14} /> SIGINT: CONTACTO
                </div>
                {profile.phone_numbers?.map((p, i) => (
                  <div key={i} className="text-sm mb-2">
                    <span className="text-white">{p.number}</span>
                    <span className="ml-2 text-cyan-400 text-xs bg-cyan-900/30 px-2 py-1 rounded">
                      Tag: {p.tag}
                    </span>
                  </div>
                ))}
              ...
              <div className="bg-neutral-900 border border-neutral-800 p-4 rounded-lg">
                <div className="text-xs text-neutral-500 flex items-center gap-1 mb-2">
                  <Briefcase size={14} /> LABORAL (IPS/SFP)
                </div>
                <div className="text-sm text-white">
                  {profile.employer || 'Sin datos laborales'}
                </div>
                {profile.ips_status && (
                  <div className="text-xs text-neutral-400 mt-1">
                    IPS: {profile.ips_status}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-5 h-80 flex flex-col">
              ...
              <div className="flex-grow border border-neutral-800 bg-neutral-950 rounded flex items-center justify-center relative overflow-hidden">
                ...
                <div className="absolute w-16 h-16 bg-cyan-900/50 rounded-full border border-cyan-500 flex items-center justify-center text-xs z-10">
                  Target
                </div>
                <div className="absolute top-10 left-10 w-12 h-12 bg-purple-900/50 rounded-full border border-purple-500 flex items-center justify-center text-[10px] z-10">
                  IPS
                </div>
                <div className="absolute bottom-10 right-10 w-12 h-12 bg-green-900/50 rounded-full border border-green-500 flex items-center justify-center text-[10px] z-10">
                  Auto
                </div>
                ...
                <span className="absolute bottom-2 right-2 text-xs text-neutral-600 z-10">
                  [Interactive Graph Rendered Here]
                </span>
              </div>
            </div>
          </div>
      ...
      {activeTab === 'patrimonio' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-5">
            <h3 className="text-cyan-500 font-bold mb-4 flex items-center gap-2">
              <Car size={18} /> PARQUE AUTOMOTOR (DNRPA)
            </h3>
            <div className="space-y-3">
              {profile.assets
                ?.filter(
                  (a) =>
                    a.asset_type === 'Vehiculo' || a.asset_type === 'Vehículo'
                )
                .map((a, i) => (
                  <div
                    key={i}
                    className="p-3 bg-neutral-950 border border-neutral-800 rounded flex justify-between items-center"
                  >
                    <span className="text-sm">
                      {a.description} ({a.identifier})
                    </span>
              ...
              {(!profile.assets ||
                profile.assets.filter((a) => a.asset_type.includes('Veh'))
                  .length === 0) && (
                <div className="text-sm text-neutral-500">
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-5">
            <h3 className="text-cyan-500 font-bold mb-4 flex items-center gap-2">
              <Home size={18} /> INMUEBLES (CATASTRO)
            </h3>
            <div className="space-y-3">
              {profile.assets
                ?.filter((a) => a.asset_type === 'Inmueble')
                .map((a, i) => (
                  <div
                    key={i}
                    className="p-3 bg-neutral-950 border border-neutral-800 rounded flex justify-between items-center"
              ...
              {(!profile.assets ||
                profile.assets.filter((a) => a.asset_type === 'Inmueble')
                  .length === 0) && (
                <div className="text-sm text-neutral-500">
      ...
      {activeTab === 'digital_footprint' && (
        <div className="grid grid-cols-1 gap-6">
          <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-5">
            <h3 className="text-cyan-500 font-bold mb-4 flex items-center gap-2">
              <Search size={18} /> PERFILES SOCIALES
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {profile.social_profiles?.map((s, i) => (
                ...
                >
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="text-xs text-neutral-500 hover:text-cyan-400 truncate block"
                  >
                    {s.url}
                  </a>
                </div>
              ...
              {(!profile.social_profiles ||
                ...
                <div className="text-sm text-neutral-500">
                  No se encontraron perfiles sociales.
                </div>
              )}
            </div>
          </div>
          <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-5">
            ...
            <div className="space-y-2">
              {profile.emails?.map((e, i) => (
                <div
                  key={i}
                  className="text-sm text-neutral-300 p-2 bg-neutral-950 border border-neutral-800 rounded"
                >
                  {e}
                </div>
              ))}
      ...
      {activeTab === 'noticias_breaches' && (
        <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-5">
          <h3 className="text-red-400 font-bold mb-4 flex items-center gap-2">
            <ShieldAlert size={18} /> ALERTAS Y FILTRACIONES
          </h3>
          <div className="space-y-4">
            {profile.threats?.map((t, i) => (
              <div
                key={i}
                className={`p-4 border rounded-lg ${t.severity > 7 ? 'bg-red-950/20 border-red-900/50' : 'bg-yellow-950/20 border-yellow-900/50'}`}
              >
                <div className="flex items-center gap-2 mb-1">
                  <AlertTriangle
                    size={16}
                    className={
                      t.severity > 7 ? 'text-red-500' : 'text-yellow-500'
                    }
                  />
                  <span className="text-xs font-bold uppercase tracking-wider text-neutral-400">
                    Severidad: {t.severity}/10 | {t.category}
                  </span>
                ...
                {t.url_source && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="text-xs text-cyan-500 hover:underline ml-6 mt-2 block"
                  >
                    Ver fuente
                  </a>
                )}
            ...
            {(!profile.threats || profile.threats.length === 0) && (
              <div className="text-sm text-neutral-500">
                No se encontraron amenazas o filtraciones.
              </div>
            )}
          </div>
        </div>
